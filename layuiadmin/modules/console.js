layui.define((exports) => {
  ...
  // 初始化数据
  admin.req({
    url: setter.base + 'json/home/console.json',
    success(res) {
      data = res.data
    },
  })
  ...
  layui.use(['carousel', 'echarts'], () => {
    let carousel = layui.carousel,
      ...
      renderDataView = (index) => {
        ...
        echartsApp[index].setOption(data.echarts[index])
    ...
    // 监听数据概览轮播
    carousel.on('change(layadmin-index-dataview)', (data) => {
  ...
  layui.use('table', () => {
    ...
    table.render({
      ...
      url: setter.base + 'json/home/top-search.json', // 数据接口
      page: true, // 是否开启分页
      // 表头
      cols: [
        [
    ...
    table.render({
      ...
      url: setter.base + 'json/home/top-card.json',
      page: true,
      cellMinWidth: 120,
      cols: [
        [
