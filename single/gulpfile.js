...
var argv = require('minimist')(process.argv.slice(2), {
  ...
  task = {
    ...
    mv: function () {
      ...
      gulp
        .src('./src/lib/extend/echarts.js')
        .pipe(gulp.dest(destDir + '/lib/extend'))

      gulp.src('./src/style/res/**/*').pipe(gulp.dest(destDir + '/style/res'))

      return gulp.src('./src/views/**/*').pipe(gulp.dest(destDir + '/views'))
    },
  }
...
gulp.task('src', function () {
  //命令：gulp src
  return gulp.src('./dev-pro/**/*').pipe(gulp.dest('./src'))
})
...
gulp.task('release', function () {
  //命令：gulp && gulp release

  //复制核心文件
  gulp.src('./dist/**/*').pipe(gulp.dest(releaseDir + '/dist'))

  gulp.src('./src/**/*').pipe(gulp.dest(releaseDir + '/src'))
  ...
  gulp.src(['./帮助/说明.txt']).pipe(gulp.dest(releaseDir))

  //复制 gulpfile
  gulp.src(['gulpfile.js', 'package.json']).pipe(gulp.dest(releaseDir))

  //说明
  gulp.src('../pack/说明.txt').pipe(gulp.dest('../pack/layuiAdmin.pack'))
